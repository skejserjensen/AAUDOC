...
linkJob inputPath outputPath = prepareIndexFile inputPath outputPath "" ""

linkDocJob :: String -> String -> Document -> IO (ExitCode, String, String)
...
-- Helper Functions --
buildLinkJob :: String -> String -> (String -> String -> Document -> IO (ExitCode, String, String)) -> Job
buildLinkJob inputPath outputPath linkFunc = StandardJob ("Linking: " ++ inputPath ++ " => " ++ outputPath)
                    (linkFunc inputPath outputPath)

prepareIndexFile :: String -> String -> String -> String -> Document -> IO (ExitCode, String, String)
prepareIndexFile inputPath outputPath indexHeader indexFooter _ = do
                    -- Searches the specified folder for tex files and a bibliography
                    textFiles <- getFilesWithSuffixRecursive inputPath [".bib", ".tex"]
                    -- Removes the index file if it have been included in the index itself
                    let textFilesWithoutOutputFile = delete outputPath textFiles
                    -- Clusters .bib files in the same same folder into one \bibliography{} block
                    let groupedTextFilesWithoutOutputFile = groupBibFiles textFilesWithoutOutputFile "" []
                    -- Prepares each line for the index file according to the tpye of file it references
                    let prepareIndexLines = formatIndexLines . replaceWindowsPathSeparators
                    let outputFileLines = map prepareIndexLines groupedTextFilesWithoutOutputFile
                    ...
                    outputAppend indexHeader
                    mapM_ (\ line -> outputAppend $ '\n' : line) outputFileLines
                    outputAppend indexFooter
                    -- The return of (ExitSuccess, "", "") is added for a uniform interface between all jobs
                    return (ExitSuccess, "", "")
...
groupBibFiles :: [String] -> String -> [String] -> [String]
groupBibFiles [] "" acc = reverse acc
groupBibFiles [] bibAcc acc = reverse $ (init bibAcc ++ ".bib") : acc
groupBibFiles (x : xs) bibAcc acc
    | takeExtension x == ".bib" = groupBibFiles xs (bibAcc ++ dropExtension x ++ ",") acc
    | otherwise = groupBibFiles xs "" updatedAcc
      where updatedAcc = if null bibAcc
                            then x : acc
                            else x : (init bibAcc ++ ".bib") : acc
